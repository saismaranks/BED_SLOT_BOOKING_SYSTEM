...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/beddbms'
db=SQLAlchemy(app)
...
class User(UserMixin,db.Model):
    id=db.Column(db.Integer,primary_key=True)
    uniqueid=db.Column(db.String(20),unique=True)
...
def signup():
    if request.method=="POST":
        uniqueid=request.form.get('srf')
        email=request.form.get('email')
        dob=request.form.get('dob')
        # print(uniqueid,email,dob)
        encpassword=generate_password_hash(dob)
        user=User.query.filter_by(uniqueid=uniqueid).first()
        emailUser=User.query.filter_by(email=email).first()
        if user or emailUser:
            flash("Email or srif is already taken","warning")
            return render_template("usersignup.html")
        new_user=db.engine.execute(f"INSERT INTO `user` (`uniqueid`,`email`,`dob`) VALUES ('{uniqueid}','{email}','{encpassword}') ")
...
def hospitalUser():
    ...
    if('user' in session and session['user']=="admin"):
        ...
        if request.method=="POST":
            ...
            # mail.send_message('BED BOOKING CENTER',sender=params['gmail-user'],recipients=[email],body=f"Welcome thanks for choosing us\nYour Login Credentials Are:\n Email Address: {email}\nPassword: {password}\n\nHospital Code {hcode}\n\n Do not share your password\n\n\nThank You..." )
...
@login_required
def pdetails():
    code=current_user.uniqueid
    print(code)
    data=Bookingpatient.query.filter_by(uniqueid=code).first()
...
def slotbooking():
    ...
    if request.method=="POST":
        ...
        checkpatient=Bookingpatient.query.filter_by(uniqueid=uniqueid).first()
        if checkpatient:
            flash("already unique id is registered ","warning")
        ...
        if check!=None:
            if(seat>0 and check):
                res=Bookingpatient(uniqueid=uniqueid,bedtype=bedtype,hcode=hcode,spo2=spo2,pname=pname,pphone=pphone,paddress=paddress)
                db.session.add(res)
                db.session.commit()
                flash("Slot is Booked kindly Visit Hospital for Further Procedure","success")
                return render_template("booking.html",query=query,query1=query1)
            else:
                flash("Bed slot not available","danger")
                return render_template("booking.html",query=query,query1=query1)
        else:
